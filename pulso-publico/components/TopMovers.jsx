...
export default function TopMovers({ tableItems = [], prevRankMap = new Map(), prevDateUsed = '' }) {
  if (!Array.isArray(tableItems) || tableItems.length === 0) return <div style={{ fontSize: 12, opacity: 0.8 }}>Sem dados.</div>;
  if (!prevDateUsed || !(prevRankMap instanceof Map) || prevRankMap.size === 0) return <div style={{ fontSize: 12, opacity: 0.8 }}>Sem comparação disponível (não há dados do dia anterior).</div>;
